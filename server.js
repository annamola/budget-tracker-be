...
app.get("/server/run_tutorial_precheck", async (req, res, next) => {
  ...
  try {
    if (
      ...
      res.json({
        status: "error",
        errorMessage: `We didn't find a client ID in the .env file. Please make sure to copy 
      ...
      typeof plaidConfig !== "undefined" &&
      plaidConfig.basePath.indexOf("sandbox") < 0
    ) {
      res.json({
        status: "error",
        errorMessage: `You're using a non-sandbox environment in a publicly-accessible URL that has no real sign-in system and anybody can access your user's info. This is probably a terrible idea.`,
      });
      return;
    } else {
